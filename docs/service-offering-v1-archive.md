# 크몽 서비스 설명 원본

bounce-cleaner-pro의 크몽 등록 서비스 설명 원문. 랜딩 페이지(`src/components/`) 카피의 정본이며, 가격·패키지 구성·FAQ가 변경될 때 이 문서와 함께 동기화되어야 함.

---

## 1. 인트로
대량 메일 발송, 왜 안 될까요?
바운스율이 5%를 넘는 순간
대부분의 플랫폼이 계정을 정지합니다.

## 2. 바운스란?
바운스(Bounce)란 무엇인가요?
이메일을 보냈는데 상대방 메일함에 도달하지 못하고
되돌아오는 것을 바운스라고 합니다.

[하드 바운스]
존재하지 않는 이메일 주소
→ 영구적으로 도달 불가
→ 방치하면 계정 정지

[소프트 바운스]
메일함 용량 초과, 서버 일시 오류
→ 일시적 도달 불가
→ 반복되면 하드 바운스로 전환

## 3. 문제 제기
다른 플랫폼도 바운스를 자동으로 걸러줍니다.
하지만 바운스율이 5%를 넘으면 계정을 정지합니다.
저희는 다릅니다.
바운스율에 관계없이 계정 차단 없이
계속 발송할 수 있습니다.

발송하면서 바운스를 자동으로 수집하고
다음 발송 시 자동으로 제외합니다.
갈수록 깨끗해지는 리스트를 경험하세요.

## 4. 서비스 소개
저희가 해드리는 것

① 바운스 정리 (1단계 — 필수)
저희 발송 인프라로 오염된 리스트를 발송해
하드/소프트 바운스를 분류하고
유효한 주소만 추려드립니다.

② 지속 발송 (2단계 — 바운스 정리 완료 후)
정화된 리스트로 계속 발송하실 수 있습니다.
저희 발송 인프라를 계속 사용하시거나,
Amazon SES, Mailgun, Mailjet, Postmark,
SendGrid, SparkPost 등 타 발송 인프라를
연결하여 사용하실 수 있습니다.

## 5. 패키지
* 플랫폼 = 발송 프로그램 / 인프라 = 이메일 서버

[1단계 — 필수]
바운스 정리 + 저희 플랫폼 + 저희 인프라 발송
29만원/월

↓ 바운스 정리 완료 후

[2단계 — 선택]
저희 플랫폼 + 저희 인프라 계속 사용
29만원/월

저희 플랫폼 + 타 발송 인프라로 전환
15만원/월

* 바운스 정리는 반드시 저희 발송 인프라로 진행됩니다.
오염된 리스트를 타 발송 인프라로 직접 발송하면
해당 서비스 계정이 차단될 수 있기 때문입니다.

## 6. FAQ
Q. 저희 서비스는 바운스율 제한이 없나요?
A. 네, 바운스율에 관계없이 계정 차단 없이
계속 발송하실 수 있습니다.

Q. 바운스 정리 후에도 계정이 정지될 수 있나요?
A. 네, 가능합니다. 바운스율과 스팸 신고율은
별개의 문제입니다.
바운스율은 저희 서비스로 해결할 수 있지만,
콘텐츠로 인한 스팸 신고율 문제는
서비스 범위 외입니다.

Q. 월 최대 발송 한도는 얼마인가요?
A. 도메인 기준 월 한도는 아래와 같습니다.
- 서버 최대 성능 : 약 200만건
- 실제 발송 가능량과 도달률은 도메인·발신자 평판과 수신 서버 상태에 따라 결정되며, 평판이 높아질수록 서버 최대 성능까지 확장 가능합니다.

## 7. 주의사항
※ 발송 결과는 수신자 메일 서버 정책에 따라
달라질 수 있습니다.
※ 바운스율과 스팸 신고율은 별개의 문제입니다.
콘텐츠로 인한 스팸 신고율 문제는
서비스 범위 외입니다.

## 8. 끝맺음
지금 바로 바운스를 정리하세요.
리스트가 깨끗해야 어디서든 발송할 수 있습니다.
